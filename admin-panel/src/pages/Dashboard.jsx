...
const Dashboard = () => {
  ...
  // Convert options to state so they can be updated
  const [facilitiesOptions, setFacilitiesOptions] = useState([
    { value: "Cement Road", label: "Cement Road" },
    { value: "Water line with water", label: "Water line with water" },
    {
      value: "Underground Electrification",
      label: "Underground Electrification",
    },
    { value: "Sewage/Drainage Line", label: "Sewage/Drainage Line" },
    { value: "Garden", label: "Garden" },
    { value: "Other", label: "Other" },
  ]);

  const [sonderStandardOptions, setSonderStandardOptions] = useState([
  ...
  const handleEdit = (projectId) => {
    ...
    // Clear custom input states when starting to edit
    setCustomFacility("");
  ...
  const handleUpdate = async () => {
    try {
      ...
      editFormData.sonderStandard.forEach((s) =>
        data.append("sonderStandard[]", s)
      );
      ...
      const res = await axios.put(
        `https://aradhya-infra-e57v.vercel.app/api/projects/${editingProject}`,
        data,
        {
          headers: {
      ...
      <div className="pt-20 md:pt-8 md:ml-64 p-8 w-full min-h-screen bg-gray-100">
        ...
        <div className="flex flex-wrap gap-8">
          {projects.map((project) => (
            <div
              key={project._id}
              className="text-[12px] md:text-[16px] w-[320px] lg:w-[500px] p-4 rounded shadow-xl"
            >
              {editingProject === project._id ? (
                <>
                  <input
                    ...
                    className="border p-1 md:p-2 w-full mb-2"
                    placeholder="Title"
                  />
                  <input
                    name="location"
                  ...
                  <div className="mb-2">
                    ...
                    <Select
                      ...
                      value={
                        areaOptions.find(
                          (opt) => opt.value === editFormData.area
                        ) || null
                      }
                      onChange={(selected) => {
                    ...
                    <div className="flex flex-wrap gap-3 md:gap-6 justify-start my-2 md:my-4 mx-3 md:mx-5 ">
                      {editFormData.images.map((img, idx) => (
                        <div key={idx} className="flex flex-col ">
                          <label className="md:text-sm font-medium block mb-1 md:mb-2">
                            Image {idx + 1}
                          </label>
                          <input
                            type="file"
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      value={facilitiesOptions.filter((opt) =>
                        editFormData.facilities.includes(opt.value)
                      )}
                      onChange={(selected) => {
                          ...
                          onClick={() => {
                            if (customFacility.trim()) {
                              ...
                              const updatedFacilities =
                                editFormData.facilities.filter(
                                  (f) => f !== "Other"
                                );
                              setEditFormData((prev) => ({
                                ...prev,
                              ...
                              // Add the new custom facility to the options list
                              const newOption = {
                                value: customFacility.trim(),
                                label: customFacility.trim(),
                              };
                              setFacilitiesOptions((prev) => {
                                // Check if the option already exists
                                if (
                                  !prev.find(
                                    (opt) => opt.value === customFacility.trim()
                                  )
                                ) {
                  ...
                  <div className="mt-2 md:mt-4">
                    ...
                    <Select
                      ...
                      value={sonderStandardOptions.filter((opt) =>
                        editFormData.sonderStandard.includes(opt.value)
                      )}
                      onChange={(selected) => {
                        const selectedValues = selected
                          ? selected.map((opt) => opt.value)
                    ...
                    {showCustomSonderInput && (
                      <div className="mt-3">
                        <input
                          ...
                          onChange={(e) =>
                            setCustomSonderStandard(e.target.value)
                          }
                          ...
                          onClick={() => {
                            if (customSonderStandard.trim()) {
                              ...
                              const updatedSonderStandard =
                                editFormData.sonderStandard.filter(
                                  (s) => s !== "Other"
                                );
                              setEditFormData((prev) => ({
                                ...prev,
                              ...
                              // Add the new custom sonder standard to the options list
                              const newOption = {
                                value: customSonderStandard.trim(),
                                label: customSonderStandard.trim(),
                              };
                              setSonderStandardOptions((prev) => {
                                // Check if the option already exists
                                if (
                                  !prev.find(
                                    (opt) =>
                                      opt.value === customSonderStandard.trim()
                                  )
                                ) {
                                  // Insert before "Other" option
                                  const withoutOther = prev.filter(
                  ...
                  <div className="flex gap-2 md:gap-4 mt-2 md:mt-4 text-[13px] md:text-[16px]">
                    <button
                      ...
                      className="text-green-600 hover:text-green-700 bg-green-200 hover:bg-green-300 px-1.5 md:px-2 py-0.5 md:py-1 rounded-sm md:rounded-md font-medium transition duration-200"
                    >
                      Save
                    </button>
                    <button
                      onClick={() => {
                        setEditingProject(null);
                        // Clear custom input states when canceling
                        setCustomFacility("");
                        setShowCustomFacilityInput(false);
                        setCustomSonderStandard("");
                        setShowCustomSonderInput(false);
                      }}
                      className="text-gray-600 hover:text-gray-700 bg-gray-300 hover:bg-gray-400 px-1.5 md:px-2 py-0.5 md:py-1 rounded-sm md:rounded-md font-medium transition duration-200"
                    >
                      Cancel
                    </button>
                  </div>
                ...
                <>
                  ...
                  <p className=" text-gray-600 mt-1">
                    ⭐ {project.rating}/5 | {project.reviews}+ reviews
                  </p>
                  ...
                  <p className="mt-1">
                    Sonder Standard: {project.sonderStandard?.join(", ")}
                  </p>

                  <div className="flex gap-2 md:gap-4 mt-3 md:mt-4 text-[12px] md:text-[14px] ">
                    <button
                      onClick={() => handleEdit(project._id)}
                      className="text-yellow-600 hover:text-yellow-700 bg-yellow-200 hover:bg-yellow-100 px-1.5 md:px-2 py-0.5 md:py-1 rounded-sm md:rounded-md font-medium transition duration-200"
                    >
                      Edit
                    </button>
                    <button
                      onClick={() => handleDelete(project._id)}
                      className="text-red-600 hover:text-red-700 bg-red-200 hover:bg-red-100 px-1.5 md:px-2 py-0.5 md:py-1 rounded-sm md:rounded-md font-medium transition duration-200"
                    >
                      Delete
                    </button>
                  </div>
