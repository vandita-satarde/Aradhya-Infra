      ...
      <div className="">
        ...
        <div className="md:py-5">
          ...
          <div className=" flex flex-col md:flex-row justify-center items-center gap-5 lg:gap-10 py-5 lg:py-15 bg-white relative">
            <img
              src={
                mainImage || "https://via.placeholder.com/870x413?text=No+Image"
              }
              alt="Main"
                ...
                project.images.slice(1).map((image, index) => {
                  ...
                  return (
                    <img
                      ...
                      src={
                        image ||
                        "https://via.placeholder.com/348x196?text=No+Image"
                      }
                      alt={`Side ${realIndex}`}
          ...
          <div className="px-5 md:px-15 lg:px-34 py-8 md:py-4 lg:py-10 text-[#073937]">
            <p className="text-[23px] md:text-[30px] lg:text-[40px] font-[abril] font-bold ">
              {project.title}{" "}
            </p>
            <div className="flex flex-col md:flex-row gap-6 md:gap-20 ">
              <div className="md:w-1/2">
                <p className="text-[12px] md:text-[13px] lg:text-[16px]">
                  ⭐{project.rating}/5 Superb ({project.reviews} reviews){" "}
                </p>
                <p className=" text-[13px] md:text-[15px] lg:text-[16.88px] py-2">
                  Description
                </p>
                <div className="text-[12px] m-5 flex gap-4 md:gap-3 lg:gap-10 ">
                  <p className="bg-[#F3ECDC] text-[#050810] px-3">Retail</p>
                  <p className="bg-[#F3ECDC] px-3"> Office Space</p>
                </div>
              ...
              <div className="md:w-1/2 ">
                {/* Only show the container if there are facilities or sonder standards */}
                {((project.facilities && project.facilities.length > 0) ||
                  (project.sonderStandard &&
                    project.sonderStandard.length > 0)) && (
                  <div className="flex flex-row gap-6 md:gap-7 lg:gap-10">
                    {/* Facilities Section - Only show if facilities exist */}
                    {project.facilities && project.facilities.length > 0 && (
                        ...
                        <ul className="text-[12px] md:text-[15.5px] flex flex-col gap-1 md:gap-4">
                          {project.facilities.map((item, i) => (
                            <li key={i} className="flex items-center gap-2">
                              <span className="text-md text-[#073937]">
                                {facilityIcons[item] || "🏢"}
                              </span>
                              <span>{item}</span>
                            </li>
                    ...
                    {/* Sonder Standard Section - Only show if sonder standards exist */}
                    {project.sonderStandard &&
                      project.sonderStandard.length > 0 && (
                        <div className="flex flex-col gap-1 md:gap-2 lg:gap-5">
                ...
                <a href="/enquiry">
                  <button className="mt-10 text-[14px] md:text-[18px] w-[130px] md:w-[250px] lg:w-[590px] h-[40px] md:h-[52px] lg:h-[60px] bg-black text-[#F3ECDC] cursor-pointer ">
                    ENQUIRY
                  </button>
                </a>
              </div>
